...
# ── Período de análise ────────────────────────────────────────────────────────
import os as _os
DATA_INICIO = datetime(int(_os.environ.get("GAP_INI","2016")), int(_os.environ.get("GAP_MI","1")), 1)
DATA_FIM    = datetime(int(_os.environ.get("GAP_FF","2017")), int(_os.environ.get("GAP_MF","12")), 28)
...
ARQUIVO_CSV = PASTA_BASE / ("base_textual_wordpress_TESTE.csv" if MODO_TESTE
                            else _os.environ.get("GAP_OUT","base_textual_petr4_wordpress_2016_2017.csv"))
ARQUIVO_LOG = PASTA_BASE / _os.environ.get("GAP_LOG","coleta_wordpress_gap.log")
