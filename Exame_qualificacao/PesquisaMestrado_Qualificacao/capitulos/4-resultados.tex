...
Este capítulo apresenta e discute os resultados obtidos sobre o corpus completo, dotado de marcação temporal real. A exposição segue a ordem do método. Inicia pela descrição exploratória do corpus de notícias e pela distribuição do sentimento. Passa à modelagem econométrica do risco e ao seu diagnóstico. Chega à previsão de direção, acompanhada de linhas de base e de testes de significância, e aprofunda-se nas análises de robustez: o efeito da ponderação do sentimento, a causalidade de Granger, a avaliação da volatilidade, o estudo de refinamento e a análise de ablação. Encerra com a discussão da distinção entre mercado e ativo, com os estudos de refinamento e validação do componente de sentimento, e com a verificação das hipóteses. Todos os números provêm de execução reproduzível, e a sua interpretação é conduzida com a cautela que a quase-eficiência do mercado impõe.
...
O gráfico quantil-quantil da Figura~\ref{fig:qqplot} confirma o diagnóstico de não normalidade de outra forma. Se os retornos seguissem uma distribuição normal, os pontos se alinhariam sobre a reta de referência; o que se observa, em vez disso, é um desvio acentuado nas extremidades, com os pontos das caudas afastando-se da reta, sinal inequívoco de que os eventos extremos ocorrem com frequência muito maior do que a normal prevê. Esse comportamento reforça a adequação da distribuição \textit{t}-Student adotada no GARCH e da abordagem quantílica empregada na análise do efeito do sentimento.

\begin{figure}[htpb]
\centering
\caption{Gráfico quantil-quantil dos log-retornos da PETR4 contra a distribuição normal. O afastamento dos pontos em relação à reta nas extremidades evidencia as caudas pesadas.}
\vspace{0.2cm}
\includegraphics[width=0.55\textwidth]{figuras/fig_qqplot.png}
\label{fig:qqplot}
\vspace{0.2cm}
{\small Fonte: Elaborado pelo autor.}
\end{figure}
...
\textbf{XGBoost (Data Fusion)} & \textbf{52,22\%} & \textbf{54,64\%} & \textbf{56,91\%} & \textbf{0,514} \\ \hline
\end{tabular}%
}
\vspace{0.2cm}
{\small \\ Fonte: Elaborado pelo autor.}
...
O melhor preditor entre os quatro é o XGBoost com fusão de dados, que atinge cinquenta e dois vírgula dois por cento de acurácia. Um resultado, porém, exige honestidade interpretativa e reorienta a leitura de todo o capítulo: nesta janela ampliada, a classe majoritária, que consiste em prever sempre a alta, alcança cinquenta e três vírgula um por cento, de modo que nenhum dos classificadores supera esse patamar trivial. A extensão do período para o intervalo de dois mil e dezesseis a dois mil e vinte e seis, que incorpora regimes muito heterogêneos, como o choque da pandemia de dois mil e vinte e o episódio geopolítico de dois mil e vinte e seis, tornou a tarefa direcional substancialmente mais difícil do que na janela mais curta e homogênea examinada na versão de qualificação. Convém, ainda, uma observação metodológica sobre a interpretação das métricas. O F1-Score elevado do SVM, de aproximadamente sessenta e seis por cento, não deve ser lido como sinal de bom desempenho: ele decorre de o modelo prever quase sempre a classe de alta, majoritária, o que infla a revocação dessa classe à custa da outra. Esse é justamente o tipo de artefato que a leitura conjunta de várias métricas e das linhas de base permite desmascarar, e que reforça a opção por reportar a acurácia ao lado do F1, da precisão e da AUC, em vez de uma única métrica isolada.

Para aferir a relevância estatística, aplicaram-se dois testes formais, escolhidos por responderem a perguntas distintas. O teste binomial avalia se o número de acertos do modelo de fusão, trezentos e quarenta em seiscentos e cinquenta e dois, supera o que se esperaria do acaso, sob a hipótese de que cada previsão tem cinquenta por cento de chance de estar correta. O valor de probabilidade obtido, de aproximadamente zero vírgula vinte e quatro, situa-se muito acima do limiar de cinco por cento, o que não permite rejeitar a hipótese de que o desempenho seja fruto do acaso. O teste de McNemar, por sua vez, é um teste pareado, próprio para comparar dois classificadores sobre exatamente os mesmos dados, pois isola a contribuição do sentimento ao concentrar-se apenas nos pregões em que os dois modelos discordam. O modelo de fusão acertou cinquenta e dois casos em que o modelo de preços errou, contra cinquenta e três no sentido oposto, com valor de probabilidade próximo da unidade, o que indica ausência de diferença estatística entre incluir ou não o sentimento na previsão da direção. Em conjunto, os dois testes conduzem a uma conclusão franca, deliberadamente distante da linguagem conclusiva que a banca criticou na versão anterior: na janela ampliada, a hipótese H1 não se confirma, pois o sentimento não agrega poder preditivo estatisticamente distinguível do acaso à previsão da direção. Longe de invalidar a pesquisa, esse resultado reorienta a sua contribuição para o canal em que o sinal do sentimento se mostra robusto, a volatilidade, como as seções seguintes demonstram, e é plenamente coerente com a forma semiforte da hipótese de eficiência dos mercados, que prevê baixa previsibilidade direcional, sobretudo em um horizonte longo e diversificado como o aqui examinado.
...
A matriz de confusão detalha a distribuição dos acertos e dos erros. Dos duzentos e quarenta e quatro pregões de baixa, o modelo acertou cento e trinta e um, e dos duzentos e cinquenta e três pregões de alta, acertou cento e quarenta, o que totaliza os duzentos e setenta e um acertos que correspondem à acurácia de cinquenta e quatro vírgula cinco por cento. Os erros distribuem-se de forma relativamente equilibrada entre as duas classes, o que indica que o modelo não privilegia indevidamente uma direção em detrimento da outra. Esse equilíbrio é desejável, pois um classificador que simplesmente apostasse sempre na alta, classe ligeiramente majoritária, atingiria apenas o patamar da classe majoritária, inferior ao obtido. A leitura da matriz de confusão é, ademais, um antídoto contra a interpretação ingênua de métricas isoladas, como o F1 inflado do SVM discutido anteriormente: ela expõe, de forma transparente, onde o modelo acerta e onde erra, e revela que o ganho do XGBoost se distribui pelas duas classes, e não se concentra artificialmente em uma delas. A área sob a curva ROC de zero vírgula cinco quatro cinco, por sua vez, deve ser lida na escala apropriada: embora pareça próxima do valor de meio que caracteriza o acaso, qualquer valor consistentemente acima de meio, sobre um conjunto de teste independente e em um ativo líquido, já representa uma violação da forma semiforte da eficiência, e o seu valor econômico depende menos da magnitude absoluta do que da capacidade de ordenar corretamente os pregões nas faixas extremas, justamente onde a regressão quantílica mostrou residir o sinal.
...
\end{tabular}
\vspace{0.2cm}
{\small \\ Fonte: Elaborado pelo autor. Suíte de refinamento de julho de 2026, sobre o recorte de notícias publicadas após o fechamento. Ver, adiante, a revisão desta leitura.}
\end{table}

A leitura inicialmente extraída desta tabela foi a de que o sinal preditivo residiria na \emph{intensidade} do sentimento --- no grau de certeza do modelo --- e não na contagem de polaridades, uma vez que o saldo de votos retornava ao patamar do acaso. Essa interpretação não se sustentou sob reexame, e a sua revisão é reportada a seguir, na Seção~\ref{sec:revisao_ponderacao}.
...
Schumaker e Chen \cite{schumaker_quantitative_2009} & EUA, preço 20 min após a notícia & 71,2\% (melhor entre esquemas; 5 semanas) \\
Bollen et al. \cite{bollen_twitter_2011} & EUA, direção do índice DJIA & 86,7\% = 13 de 15 pregões; não replicado \\
...
A comparação revela tanto avanços quanto pontos de aparente desvantagem, que convém discutir com franqueza. À primeira vista, a acurácia absoluta de cinquenta e quatro vírgula cinco por cento obtida aqui é inferior à de estudos como o de \citeonline{bollen_twitter_2011}, com oitenta e seis vírgula sete por cento, ou o de \citeonline{barak_fusion_2017}, com mais de oitenta por cento. Essa diferença, contudo, não decorre de inferioridade do método, e sim da incomparabilidade dos cenários --- e, em um dos casos, da própria fragilidade do número de referência. Convém examinar cada um.

O resultado de \citeonline{bollen_twitter_2011} é o mais citado da área e merece exame detido, porque a sua base empírica é substancialmente mais estreita do que a cifra sugere. Os autores treinam o modelo sobre o período de 28 de fevereiro a 28 de novembro de 2008 e avaliam a previsão da direção do índice Dow Jones no intervalo de 1 a 19 de dezembro de 2008 --- \textbf{quinze pregões}. A acurácia de oitenta e seis vírgula sete por cento corresponde, portanto, a \textbf{treze acertos em quinze dias}. A isso somam-se duas circunstâncias. A primeira é que o alvo é um índice agregado, e não uma ação individual: a diversificação elimina o ruído idiossincrático que constitui a maior parte da variação de um papel isolado. A segunda, e decisiva, é que o resultado \textbf{não sobreviveu à replicação}. \citeonline{lachanski_shy_2017} reconstruíram a série e a estenderam para trás, de modo a incluir 2007, e não encontraram evidência de que as medidas de humor auxiliem a previsão fora da amostra, atribuindo o achado original a \textit{data snooping} e ao viés de comparações múltiplas decorrente da alta dimensionalidade do espaço de estados de humor testados. Registre-se, ainda, que dos sete estados de humor examinados por \citeonline{bollen_twitter_2011}, apenas um --- a dimensão de calma --- apresentou poder preditivo, ao passo que a polaridade positiva e negativa, que é a medida adotada nesta dissertação e na maior parte da literatura, \textbf{não} o apresentou.

O trabalho de \citeonline{schumaker_quantitative_2009} não padece de fragilidade análoga, mas mede outra coisa. O sistema estima o preço \textbf{vinte minutos após a divulgação} de cada notícia, sobre cinco semanas de dados. Trata-se, portanto, da mensuração da \emph{reação} imediata do mercado à notícia, e não da antecipação do pregão seguinte. A distinção é exatamente a que a Seção~\ref{sec:auditoria_noticia} estabelece entre os horizontes $P_0$ e $P_1$: também nesta pesquisa o sinal é nítido no pregão que reage e se dissolve no pregão seguinte. Os setenta e um vírgula dois por cento reportados são, ademais, o melhor resultado entre vários esquemas de particionamento do corpus.

O estudo de \citeonline{barak_fusion_2017}, por sua vez, aplica-se ao mercado de Teerã, menos líquido e possivelmente menos eficiente que a B3, dirige-se a retorno e risco em vez da direção diária, e reporta a acurácia máxima entre várias configurações. Sua técnica central --- a fusão de preditores diversos por empilhamento --- foi replicada nesta pesquisa e consta da Tabela~\ref{tab:suite_experimentos}: o \textit{stacking} alcançou 53,14\%, sem superar o \textit{baseline} de classe majoritária nem a configuração mais simples.

A previsão da direção diária de uma ação líquida, como a PETR4, sob avaliação cronológica estrita e fora da amostra, é um dos cenários mais exigentes possíveis, e a acurácia próxima de cinquenta por cento é o resultado esperado pela Hipótese de Mercados Eficientes. O contraste com os números da tabela não indica, pois, deficiência do método aqui empregado, e sim diferença de alvo, de horizonte, de amostra e de protocolo de avaliação.
...
\section{Estudos de Refinamento e Validação do Componente de Sentimento}
\label{sec:refinamento_validacao}

As seções anteriores estabeleceram o desempenho do sistema completo. Esta seção examina o
componente de sentimento em si, que constitui o primeiro elo da cadeia e cuja qualidade limita
tudo o que vem depois. A ordem da exposição segue a do próprio percurso de investigação: primeiro
os experimentos de refinamento do classificador de direção, em seguida a validação do sentimento
automático contra um padrão de referência humano e, por fim, as tentativas sucessivas de elevar
essa validação, encerradas por um resultado negativo que delimita o alcance da abordagem. Seguem-se
os achados de implementação sobre o artefato público empregado e o único experimento da série que
produziu ganho mensurável --- o qual, sintomaticamente, não atua sobre como o modelo lê cada notícia,
mas sobre quais notícias entram no índice. Encerra a seção uma auditoria direta, sem modelo
interposto, que confronta o rótulo de cada notícia com o pregão que se seguiu e delimita, em números
verificáveis, o alcance do sinal textual.

\input{capitulos/4d-experimentos-suite}
\input{capitulos/4e-filtro-relevancia}
\input{capitulos/4f-estrategia-refinamento}
\input{capitulos/4g-validacao-sentimento}
\input{capitulos/4h-experimento-encoder}
\input{capitulos/4i-adaptacao-dominio}
\input{capitulos/4j-achados-implementacao}
\input{capitulos/4k-relevancia-volatilidade}
\input{capitulos/4l-auditoria-noticia}
\input{capitulos/4m-contraste-encoders-ingles}
\input{capitulos/4n-revisao-ponderacao}
\input{capitulos/4o-macro-versus-empresa}
