...
Um ponto conceitual merece destaque por afetar toda a modelagem. A unidade de análise desta pesquisa é o pregão, ou seja, o dia de negociação, e não a notícia individual. As notícias de um mesmo dia são agregadas em um único índice diário de sentimento, de modo que o modelo produz exatamente uma previsão por pregão. Essa escolha alinha a frequência da informação textual à frequência da decisão de investimento que se deseja apoiar, que é diária. Mais de duzentas e sessenta mil notícias do corpus não constituem, portanto, duzentas e sessenta mil observações do modelo, e sim a matéria-prima a partir da qual se constroem os índices diários que alimentam aproximadamente dois mil pregões.
...
Em atenção à recomendação da banca de validar o sentimento automático por meio de rotulagem manual, construiu-se um conjunto-ouro. Trata-se de uma amostra de trezentas notícias, selecionadas de forma estratificada por classe de sentimento e por ano, de modo a cobrir todo o período e a garantir representação da classe minoritária. Essas notícias foram rotuladas manualmente, sem que o rótulo do modelo fosse exibido ao anotador, o que evita o viés de ancoragem. Contra esse padrão de referência, mediram-se a acurácia do FinBERT-PT-BR e o coeficiente \textit{kappa} de Cohen \cite{cohen_coefficient_1960}, que quantifica a concordância entre o modelo e o anotador humano para além do que se esperaria do acaso. A interpretação do \textit{kappa} segue a convenção usual: valores entre $0{,}41$ e $0{,}60$ indicam concordância moderada, entre $0{,}61$ e $0{,}80$ concordância substancial, e acima de $0{,}80$ concordância quase perfeita. A estratificação da amostra inclui um piso para a classe positiva, que é minoritária no corpus, de modo a permitir uma estimativa confiável da acurácia por classe, e registra-se um peso amostral para cada notícia, que possibilita reponderar a acurácia agregada à distribuição populacional. A separação do rótulo do modelo em arquivo distinto, inacessível ao anotador durante a tarefa, é uma salvaguarda metodológica contra o viés de ancoragem, que ocorreria se o julgamento humano fosse influenciado pela sugestão automática. Registre-se, desde já, uma limitação do procedimento: a anotação foi conduzida por um único avaliador, de modo que não há segunda anotação nem, por consequência, métrica de concordância entre anotadores. As medidas reportadas na Seção~\ref{sec:validacao_sentimento} expressam, portanto, a distância entre o classificador e esse avaliador, e não entre o classificador e um padrão validado por consenso. A implicação dessa limitação é discutida no Capítulo~\ref{chap:conclusao}.
...
\label{sec:metodologia_modelagem}
...
Esse percurso evidencia como o método distingue uma notícia negativa para o mercado de uma negativa para a empresa, questão que a banca considerou central. Vale ressaltar que, no sistema implementado, a notícia individual não gera diretamente uma previsão de preço; ela contribui, junto com todas as demais do pregão, para o índice diário de sentimento, que então alimenta o modelo. A leitura econômica setorial descrita no exemplo opera, assim, em duas instâncias complementares: na categorização que organiza o corpus para a análise de ablação, e na aplicação interativa que acompanha esta dissertação, na qual o usuário submete uma manchete e recebe, além da previsão estatística, a interpretação econômica de seu provável efeito. Essa dupla operação ilustra a distância entre o modelo estatístico, que prevê uma direção por pregão, e a ferramenta de apoio à decisão, que interpreta uma notícia isolada, ambos derivados da mesma arquitetura.
