...
| [src/modelagem/05_refinamento_modelos_petr4.py](src/modelagem/05_refinamento_modelos_petr4.py) | **Script 05** — refinamento controlado (E0–E8): engenharia de atributos, seleção, modelos alternativos, tuning e walk-forward; conclui pela parcimônia (baseline mais robusto fora da amostra) |
| [src/modelagem/06_analises_complementares_petr4.py](src/modelagem/06_analises_complementares_petr4.py) | **Script 06** — análises de robustez: ISM ponderado, baseline+significância (binomial/McNemar), causalidade de Granger (Lead-Lag) e avaliação da previsão de volatilidade (Mincer-Zarnowitz/QLIKE) |
| [src/sentimento/preparar_conjunto_ouro_petr4.py](src/sentimento/preparar_conjunto_ouro_petr4.py) · [src/sentimento/avaliar_conjunto_ouro_petr4.py](src/sentimento/avaliar_conjunto_ouro_petr4.py) | **Conjunto-ouro** — amostra estratificada (300) p/ rotulagem humana e avaliação do FinBERT-PT-BR por acurácia + **kappa de Cohen** |
...
| [docs/geradores/](docs/geradores/) | Geradores dos documentos Word **ABNT**: Etapa 1 (engenharia de dados), Etapa 2 (financeira), Etapa 3 (sentimento, **com justificativa FinBERT-PT-BR × BERTimbau e critérios**), Etapa 4 (modelagem, com evolução/tuning), **Etapa 4b (refinamento)**, Etapa 5 (inferência), **Etapa 6 (análises complementares: ISM ponderado, significância, Granger, volatilidade)**. Saída em `docs/saida/` |
| [docs/ANALISE_DE_GAPS_BANCA.md](docs/ANALISE_DE_GAPS_BANCA.md) | **Mapa de defesa** — cobertura das 24 ponderações da banca (resolvido/parcial/pendente) + sugestões de adições |
